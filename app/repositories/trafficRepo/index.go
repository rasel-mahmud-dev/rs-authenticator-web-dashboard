...
func (r *Repository) GetApiLatencyStats() ([]dto.DetailedTrafficStats, error) {
	query := `
		SELECT AVG(response_time) AS avg_response_time, route_path, count(route_path) as request_count
		FROM user_traffic
		GROUP BY route_path
	`
	rows, err := r.db.Query(query)
	...
	for rows.Next() {
		var s dto.DetailedTrafficStats
		err := rows.Scan(&s.AvgResponseTime, &s.RoutePath, &s.Count)
		if err != nil {
			log.Println("Error scanning row:", err)
			continue
		}
